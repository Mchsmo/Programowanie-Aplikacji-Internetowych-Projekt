@if (isset($paginator) && $paginator->hasPages())
    <div style="margin-top: 3em; text-align: center; display: flex; justify-content: center; gap: 0.5em; align-items: center;">
        ...
        @else
            <a href="{{ $paginator->appends(request()->except($paginator->getPageName()))->previousPageUrl() }}" class="button alt small" style="height: 2.5em; line-height: 2.5em; padding: 0 1.5em;">&laquo; Poprzednia</a>
        @endif
        ...
        @foreach ($paginator->getUrlRange(1, $paginator->lastPage()) as $page => $url)
            ...
            @else
                <a href="{{ $paginator->appends(request()->except($paginator->getPageName()))->url($page) }}" class="button alt small" style="height: 2.5em; line-height: 2.5em; width: 2.5em; padding: 0; text-align: center;">{{ $page }}</a>
            @endif
        @endforeach

        {{-- Przycisk Dalej --}}
        @if ($paginator->hasMorePages())
            <a href="{{ $paginator->appends(request()->except($paginator->getPageName()))->nextPageUrl() }}" class="button alt small" style="height: 2.5em; line-height: 2.5em; padding: 0 1.5em;">Następna &raquo;</a>
        @else
